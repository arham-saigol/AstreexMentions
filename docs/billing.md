...
When the next new mention would exceed the cap, that candidate and later candidates are not inserted. Provider results are durably staged before application, and the atomic ingestion mutation records the first unprocessed position on the current batch, returns `checkpoint: "hold"`, and pauses every active tracking source in the workspace with `pauseReason: "usage"`. It finalizes concurrent in-flight tracking runs before clearing their leases; the run that reached the cap is finalized by its owning scheduling mutation. On resume, the current and later staged batches are drained before another live provider request, without double-counting earlier items.
