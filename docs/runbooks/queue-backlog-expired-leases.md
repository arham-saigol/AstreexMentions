...
| -------------------------------------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Tracking in `trackingSources`/`providerRuns` | One-minute cron; scans up to 256 due rows per source type, then applies provider budgets | Four-minute versioned lease; expired lease can be reclaimed with a higher fencing version             | Config/error-paused sources need explicit reactivation; old `running` run records may remain after a stale worker is fenced |
| Creem `billingEvents`                        | One-minute cron; at most 16 due pending events                                           | Direct mutation retry after 30 seconds; current processing does not use optional billing lease fields | Dead event IDs are terminal                                                                                                 |
...
- Expired 60-second leases are included with pending rows in the next one-minute claim, subject to the 32-row cap.
