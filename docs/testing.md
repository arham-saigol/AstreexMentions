...
| DeepSeek analysis      | `DEEPSEEK_API_KEY` with access to `deepseek-v4-pro`                                            | Fixture transport plus `convex-test` dispatcher/action coverage with mocked `fetch`        | Deployed queue claim, provider request/run ID, atomic mention/job results, retry/dead-letter evidence; local tests are not credential proof                                       |
