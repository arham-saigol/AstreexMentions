...
This document describes the tests and commands that exist in this repository. Deterministic builds and fixture-backed tests do not prove that Clerk, Convex cloud configuration, Xquik, FetchLayer, Algolia, Vertex AI, Creem, Resend, Vercel, DNS, OAuth, payment, or mailbox delivery works with real credentials.
...
- mention analysis batch and total-output validation;
...
1. **Pure/fixture/source tests** import schema and business modules directly. They validate the 25-table schema and indexes, generic pre-codegen boundary, authorization primitives, billing lifecycle, scheduling, adapters, mention analysis contracts, digest/email rules, and frontend/backend reference contracts.
2. **`convex-test` integration tests** execute selected queries, mutations, and actions against in-memory test schemas. They cover atomic ingestion, the durable mention analysis dispatcher/action with mocked provider transport, keyword/mention behavior, customer product functions, and admin product functions.
...
- **Vertex AI Gemini** configuration, structured-output contracts, medium thinking, full-batch validation, timeouts, rate limits, secret-free logs, durable dispatch batching, category snapshots, four-minute leases, missing-configuration release, retry/dead-letter transitions, atomic result application, and provider telemetry;
- **Onboarding discovery** structured search plans and recommendations, malformed Gemini output, Gemini failures, and missing provider configuration;
...
| Vertex AI Gemini analysis | `VERTEX_AI_PROJECT_ID`, `VERTEX_AI_SERVICE_ACCOUNT_JSON`, `global`, and access to `gemini-3.5-flash-lite` | Fixture transport plus `convex-test` dispatcher/action coverage with mocked SDK transport  | Deployed queue claim, provider request/run ID, atomic mention/job results, retry/dead-letter evidence; local tests are not credential proof                                       |
...
- The mention analysis worker is covered with fixtures and mocked transport, but no automated suite proves real `gemini-3.5-flash-lite` access, quota, request support, or production output quality.
