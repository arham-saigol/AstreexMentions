...
export function SiteFooter() {
  return (
    <footer className="border-t bg-[var(--canvas-soft)]">
          ...
          <ul className="flex flex-wrap gap-x-6 gap-y-3">
            {links.map(([label, href]) => (
              <li key={label}>
                <Link
                  href={href}
                  className="hover:text-foreground text-sm text-[var(--ink-secondary)] transition-colors duration-[var(--motion-control)]"
                >
                  {label}
                </Link>
              </li>
      ...
      <div className="border-t">
        <div className="text-muted-foreground mx-auto flex w-full max-w-[1184px] flex-wrap justify-between gap-2 px-4 py-4 font-mono text-[11px] sm:px-6 lg:px-8">
