...
| `CREEM_PRODUCT_ALLOWLIST_JSON`      | Convex; maps environment-specific Creem product IDs to Astreex plans and limits. Runtime rejects duplicate plan mappings; release validation requires all three plan IDs with exact limits. |
...
| `DEEPSEEK_API_KEY`                  | Convex; credential used by the one-minute categorization worker with fixed model `deepseek-v4-pro`.                                                                                         |
| `DEEPSEEK_TIMEOUT_MS`               | Convex; optional positive timeout, default `30000`.                                                                                                                                         |
