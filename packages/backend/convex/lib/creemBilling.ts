...
export type CreemBillingEventIdentity = {
  provider: "creem"
  providerEventId: string
}

export interface CreemBillingEventStore<EventId> {
  findByProviderEventId(providerEventId: string): Promise<EventId | null>
  insert(input: CreemBillingEventIdentity): Promise<EventId>
}

/**
 * Call inside one Convex mutation. The indexed read and insert then share a
 * serializable transaction, making retries of one Creem event a no-op.
 */
export async function insertCreemBillingEventIdempotently<EventId>(
  store: CreemBillingEventStore<EventId>,
  providerEventId: string,
): Promise<
  | { eventId: EventId; kind: "duplicate" }
  | { eventId: EventId; kind: "inserted" }
> {
  const normalizedEventId = providerEventId.trim()
  if (normalizedEventId.length === 0) {
    throw new TypeError("Creem event id must be a non-empty string")
  }

  const existing = await store.findByProviderEventId(normalizedEventId)
  if (existing !== null) {
    return { eventId: existing, kind: "duplicate" }
  }
