...
import {
  applyCategorizationBatchReference,
  failCategorizationBatchReference,
  loadCategorizationBatchContextReference,
  releaseCategorizationBlockedConfigurationReference,
  startCategorizationProviderRunReference,
} from "./internal"
...
export const executeCategorizationBatch = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (context.state === "invalid_batch") {
      return await ctx.runMutation(failCategorizationBatchReference, {
        ...args,
        durationMs: 0,
    ...
    try {
      ...
      return await ctx.runMutation(applyCategorizationBatchReference, {
        ...args,
