...
function Input({
  ...
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "border-input text-foreground selection:text-foreground flex h-[var(--control-h)] w-full min-w-0 rounded-[var(--radius-sm)] border bg-[var(--surface-inset)] px-3 py-2 text-[13px] shadow-[var(--shadow-control)] transition-[border-color,box-shadow] duration-[var(--motion-control)] outline-none selection:bg-[var(--brand-soft)] placeholder:text-[var(--text-tertiary)] hover:border-[var(--line-strong)] disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
