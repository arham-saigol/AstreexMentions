...
- The existing DeepSeek categorizer only receives mention text and category definitions (`packages/backend/convex/categorization/internal.ts` and `packages/backend/convex/lib/deepseekCategorization.ts`). It will need keyword/company context at that prompt boundary for descriptions to influence categorization.
...
- Extend the categorization batch loader and `packages/backend/convex/lib/deepseekCategorization.ts` to include bounded workspace/company context and the descriptions of the mention’s matched keywords (not merely the tracking source). Keep prompt length bounds and existing category validation intact; use deterministic truncation when a mention has several matches.
